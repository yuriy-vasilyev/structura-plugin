...
export interface StepperStep {
   ...
   * Outranks the completed check: a step the user walked past and later
   * emptied must not sit there wearing a green tick.
   */
  dot?: boolean;
...
export interface StepperProps {
  ...
  activeIndex: number;
  /**
   * Furthest step the user has reached, for wizards that track it. Steps up
   * to it stay complete — and clickable — after the user navigates back, so
   * stepping back to review one answer doesn't strand them on it (the
   * campaign interview's only way forward was its own AI-drafting CTA,
   * 2026-09-22). Defaults to `activeIndex`, i.e. nothing ahead is reachable.
   */
  reachedIndex?: number;
  /**
   * When provided, COMPLETED steps become clickable — the ones behind the
   * active step, plus anything already reached ahead of it (see
   * `reachedIndex`). Wizards that commit per step want that review-ability;
   * ones that don't simply omit the handler. Steps the user has never
   * reached are never clickable — reaching them is the Continue gate's job.
   */
...
export const Stepper: FC<StepperProps> = ({
  ...
  const furthest = Math.max(activeIndex, reachedIndex ?? activeIndex);
  return (
    <div
      className={cn(
    ...
    >
      {steps.map((step, i) => {
        const state =
          i === activeIndex ? "active" : i <= furthest ? "done" : "upcoming";
